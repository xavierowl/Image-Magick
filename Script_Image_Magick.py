...
def obtener_histograma_3_canales(sunflowers,\
    ...
    for sunflower in sunflowers:
        ...
        orden = 'convert sunflower/'+ sunflower\
            +' -define histogram:unique-colors=true -format %c histogram:histogramas-3-canales/Histogramas-'\
            + carpeta + '/histograma.gif'
        os.system(orden)
        orden = 'convert  histogramas-3-canales/Histogramas-'\
            +carpeta+'/histograma.gif -strip -resize 200% -separate histogramas-3-canales/Histogramas-'\
            + carpeta+'/canal-%d.gif'
        os.system(orden)
...
x = 15
y = 770
documento.drawString(x,y,\
    'A continuación se presenta la información pertinente sobre el proyecto realizado en')
documento.drawString(x,y-20,'       image-magick: (T. p. = tiempo promedio)')
documento.drawString(x,y-40,'Tiempo total del procesamiento de las imagenes: '\
    +str(tiempo_total)+' s.')
documento.drawString(x,y-60,'T. p. para convetir a escala de grises: '\
    +str(conversion_a_gris_tiempo)+' s.')
documento.drawString(x,y-80,'T. p. para el histograma de las imágenes originales: '\
    +str(histograma_3_canales_tiempo)+' s.')
documento.drawString(x,y-100,\
    'T. p. para el histograma de las imágenes en escala de grises: '\
    +str(histograma_1_canal_tiempo)+' s.')
documento.drawString(x,y-130,'El peso original de las imágenes es:  '\
    +str(peso_original))
documento.drawString(x,y-150,'El peso en escala de grises de las imágenes es:  '\
    +str(peso_gris))
documento.setFont("Helvetica", 23)
documento.setFillColor('red')
...
documento.setFont("Helvetica", 23)
documento.setFillColor('red')
documento.drawString(210,800,'Código fuente')
documento.setFont("Helvetica", 15)
documento.setFillColor('black')
documento.drawString(x,20,'Realizado por: Byron Calva')
f = open("Script_Image_Magick.py","r")
